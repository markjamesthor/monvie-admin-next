...
export default function CustomerDetailPage() {
  ...
  const customerSessions = MOCK_DESIGN_SESSIONS.filter(
    (s) => s.customerId === customer.id,
  );
  const customerCartItems = MOCK_CART_ITEMS.filter(
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div>
              <span className="text-sm text-gray-500">총 지출액</span>
              <p className="text-sm font-bold text-gray-900">
                {fmtWon(customer.totalSpent)}
              </p>
            </div>
            <div>
      ...
      {/* Customer's design sessions table */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            디자인 세션
            <span className="ml-2 text-sm font-normal text-gray-400">
              {customerSessions.length}건
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {customerSessions.map((session) => (
                  <TableRow key={session.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="secondary"
                        className={SESSION_STATUS_COLOR[session.status]}
                      >
                    ...
                    <TableCell className="text-center">
                      <Button variant="outline" size="xs" asChild>
                        <Link href={`/design-sessions/${session.id}`}>
                          상세
                        </Link>
                      </Button>
                    </TableCell>
      ...
      {/* Customer's cart items table */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            장바구니 항목
            <span className="ml-2 text-sm font-normal text-gray-400">
              {customerCartItems.length}건
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {customerCartItems.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-right font-semibold text-gray-900">
                      {fmtWon(item.unitPrice)}
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="secondary"
                        className={CART_STATUS_COLOR[item.status]}
                      >
