...
function buildTimeline(
  session: (typeof MOCK_DESIGN_SESSIONS)[number],
): TimelineItem[] {
...
export default function DesignSessionDetailPage() {
  const params = useParams();
  const sessionId = params.id as string;

  const session = MOCK_DESIGN_SESSIONS.find((s) => s.id === sessionId);
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-900">
                {session.hasCustomText ? "예" : "아니오"}
              </p>
            </div>
            <div>
              <span className="text-sm text-gray-500">총 편집 시간</span>
              <p className="text-sm font-medium text-gray-900">
                {formatEditTime(session.totalEditTimeSeconds)}
              </p>
            </div>
          </CardContent>
        </Card>

        {/* 리커버리 정보 (if abandoned) */}
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-sm font-semibold text-gray-600">
            ...
            {session.status === "abandoned" ? (
              <>
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {session.abandonedAt
                      ? formatDateTime(session.abandonedAt)
                      : "-"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {session.recoveryEmailSentAt
                      ? formatDateTime(session.recoveryEmailSentAt)
                      : "-"}
                  </p>
                </div>
              </>
              ...
              return (
                <div key={event.id} className="relative flex gap-4 pb-6">
                  ...
                  <div className="relative z-10 flex-shrink-0 pt-0.5">
                    <Circle
                      className={`h-[18px] w-[18px] fill-current ${event.dotColor}`}
                    />
                  </div>
                  {/* Content */}
